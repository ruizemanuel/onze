...
export function FechaCard({ tournamentId, fechaNumber, round, status, sub }: FechaCardProps) {
    ...
    <div className={`flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3.5 transition hover:bg-white/5 ${dim}`}>
      <div className="min-w-0">
        <div className="flex items-baseline gap-2">
          <span className="font-display text-2xl leading-none tracking-[0.04em]">Fecha {fechaNumber}</span>
          <span className="text-[11px] uppercase tracking-[0.08em] text-white/40">GW{round}</span>
        </div>
